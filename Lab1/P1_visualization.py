...
df = pd.read_csv("time_10_1e4_M=1e3.csv")
...
plt.plot('n', 'optimize', data=df, marker='', color=palette(2), linewidth=2, label="Optimize")
plt.plot('n', 'unroll', data=df, marker='', color=palette(3), linewidth=2, label="Unroll")
...
sns.boxplot(data=df[['naive', 'optimize', 'unroll']])
...
heatmap_df = df.set_index('n')[['naive', 'optimize', 'unroll']]
...
plt.ylabel('Matrix/Vector Size (n)')
